...
def idastar_generator(initial_state, goal_state):
    initial_h = calculate_manhattan_distance(initial_state, goal_state)
    root_node = Node(initial_state, depth=0, h_cost=initial_h)

    threshold = root_node.f_cost()
    
    while True:
        stack = [(root_node, None)]
        path_states = {state_to_tuple(root_node.state)} 
        ...
        while stack:
            ...
            if children is None:
                if current_node.state == goal_state:
                    yield current_node, True, get_solution(current_node), threshold
                    return
                
                yield current_node, False, None, threshold
                if current_node.f_cost() > threshold:
                    if current_node.f_cost() < min_exceeded:
                        min_exceeded = current_node.f_cost()
                    path_states.remove(state_to_tuple(current_node.state))
                    stack.pop() 
                    continue

                children = generate_children(current_node, goal_state)
                children.reverse() 
                stack[-1] = (current_node, children)

            if children:
                child = children.pop()
                child_tuple = state_to_tuple(child.state)
                if child_tuple not in path_states:
                    path_states.add(child_tuple)
        ...
        if min_exceeded == float('inf'):
            yield None, False, None, threshold
            return
...
class PuzzleApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("8-Puzzle IDA* Search")
        ...
        self.idastar_step_generator = None
    ...
    def start_solving(self):
        ...
        try:
            ...
            self.idastar_step_generator = idastar_generator(initial, goal)
            self.process_next_idastar_step()
    ...
    def process_next_idastar_step(self):
        if not self.is_running:
            return

        try:
            current_node, success, solution, threshold = next(
                self.idastar_step_generator
            )
            self.nodes_explored += 1

            if current_node:
                self.draw_puzzle(
                    current_node.state, tile_bg="#f7fee7", text_fg="#4d7c0f"
                )
                self.status_label.config(
                    text=f"Node: {self.nodes_explored} | Ngưỡng (Threshold): {threshold} | f(n): {current_node.f_cost()}"
                )
            ...
            if success and solution:
                ...
                self.result_text.insert(
                    tk.END,
                    f"Số lượt duyệt node: {self.nodes_explored}\n",
                )
                self.result_text.insert(
                ...
                for step, node in enumerate(solution):
                    self.result_text.insert(
                        tk.END, f"Bước {step} (f = {node.f_cost()} | g = {node.depth}, h = {node.h_cost})"
                    )
                    if node.action:
                        self.result_text.insert(
            ...
            self.root.after(10, self.process_next_idastar_step)
